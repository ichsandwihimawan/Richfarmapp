...
def investView(request):
    ...
    if user.referal_by.invest_set.filter(is_active=True).exists() and user.role.role == 'user':
        bon_sponsor = nominal * 0.15
        inv_ref = user.referal_by.invest_set.get(is_active=True)
        if inv_ref.capping - bon_sponsor > 0 :
            refnya = Data_User.objects.filter(id=user.referal_by.id)
            Bonus_Sponsor.objects.create(user_invest=inv,nominal=bon_sponsor,for_user=refnya.first())
            refnya.update(total_bonus=F('total_bonus')+bon_sponsor)
            refnya_invest = refnya.first()
            refnya_invest.invest_set.filter(is_active=True).update(capping=F('capping') - bon_sponsor)
        else:
            new_bon_sponsor = inv_ref.capping
            refnya = Data_User.objects.filter(id=user.referal_by.id)
            Bonus_Sponsor.objects.create(user_invest=inv, nominal=new_bon_sponsor, for_user=refnya.first())
            refnya.update(total_bonus=F('total_bonus') + new_bon_sponsor)
            refnya_invest = refnya.first()
            refnya_invest.invest_set.filter(is_active=True).update(capping=0,is_active=False)
